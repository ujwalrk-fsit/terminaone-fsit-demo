  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      <div className="section-head" style={{ margin: 0 }}>
        <h2>Indications <span style={{ font: '400 12px var(--font-body)', color: 'var(--text-subtle)' }}>(IOI book)</span></h2>
      ...
      {list.length === 0 ? <Empty text="No indications (edge case). Create one." /> :
        <div style={{ display: 'grid', gap: 8 }}>{list.map((i) => {
          ...
          return <Card key={i._id}><div className="tnum" style={{ fontFamily: 'monospace', fontSize: 13 }}>{i._id} · {f?.fundName} · {i.numberOfUnits} units · ${i.investmentAmount.toLocaleString()} · <b style={{ color: 'var(--text-strong)' }}>{i.status}</b></div></Card>;
        })}</div>}
    </div>
  );
}
...
export function IndicationNew() {
  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', display: 'grid', gap: 16 }}>
      ...
      {step === 4 && <Card><div style={{ font: '700 15px var(--font-display)', color: 'var(--text-strong)', marginBottom: 8 }}>4. Native e-sign <span style={{ font: '400 12px var(--font-body)', color: 'var(--text-subtle)' }}>(investor leg; advisor/FM legs in Documents)</span></div>

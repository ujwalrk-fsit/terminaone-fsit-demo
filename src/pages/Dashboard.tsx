...
import { indications, funds, transfers } from '../data/sample';
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-extrabold" style={{ color: 'var(--text-strong)' }}>Dashboard <span className="text-sm font-normal" style={{ color: 'var(--text-subtle)' }}>({role})</span></h1>
      {role === 'investor' && (
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Card><div className="text-xs" style={{ color: 'var(--text-subtle)' }}>Allocated value</div><div className="text-2xl font-extrabold">${mine.filter((m) => m.status === 'ALLOCATED').reduce((a, b) => a + b.investmentAmount, 0).toLocaleString()}</div><Link to="/portfolio" className="text-sm underline">Portfolio</Link></Card>
      ...
      {(role === 'admin' || role === 'fund_manager') && (
        <Card><div className="font-bold" style={{ color: 'var(--text-strong)' }}>Approvals queue</div>
      ...
      {role === 'advisor' && <Card><div className="text-sm">Clients pending signatures: {queue.length}.</div></Card>}
      ...
      {role === 'affiliate' && <Card><div className="text-sm">Your referred funds: {funds.filter((f) => f.affiliates.length).length}.</div></Card>}
